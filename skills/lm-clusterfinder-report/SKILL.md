...
description: >-
  ...
  각 클러스터의 허브(대표) 키워드와 클러스터 간 From→To 탐색 흐름을 카드 + 허브 키워드
  요약 표 형태로 제시합니다.
...
1. **시드 키워드**: 분석 대상 (분석 시장 언어로 — kr이면 한국어). 클러스터파인더는 **단일 키워드**만 받습니다.
...
### Step 4 · 도구 호출 · MCP 도구(cluster_finder · keyword_info) · tool_call 은 **직접 발행**
...
MCP 도구(cluster_finder · keyword_info)는 optional `user_query` 파라미터를 받아 이력에 적재한다
