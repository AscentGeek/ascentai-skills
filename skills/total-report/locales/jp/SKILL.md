---
name: lm-total-report-jp
description: >-
  ListeningMind MCP の 3 ファインダー(intent_finder · path_finder · cluster_finder)を統合し、
  シードキーワードの検索意図・インテント経路・クラスタを一度に分析する「統合検索インサイト」
  リッチ HTML レポート(ダッシュボード/A4)を生成します。関連クエリ、インテントパス、クラスタを
  一つの流れでつなぎ、消費者の検索ジャーニーと総合的な検索分析をカード形式で提示します。
  「統合レポート」「統合検索インサイト」「3 ファインダー統合」「総合検索分析」を
  依頼されたときに使用してください。
...
# lm-total-report-jp — 統合検索インサイトレポート
...
ユーザーから統合検索インサイト分析を依頼されたら、**必ず `references/total-insight.md` を先に
...
`{WORKDIR}/total-insight-report.html` — 自己完結型 HTML(ダッシュボード↔A4 切替 · 印刷 PDF)。
`{WORKDIR}` は実行プロジェクトの `tmp/reports/listeningmind-total-insight-{シード}-{時刻}/` です。
...
[応答の途中 · ツール呼び出しごと]
  ② --type tool_call (intent_finder · path_finder · cluster_finder · keyword_info をそれぞれ発行)
...
SKILL_DIR=$(find ~/.claude/skills ~/.claude/plugins /mnt/skills /mnt/user-data \
    ...
    -maxdepth 4 -type d -name lm-total-report-jp 2>/dev/null \
    | grep -v '\.trash' | head -1)
...
### Step 4 · ツール呼び出し · MCP ツール(intent_finder · cluster_finder · path_finder · keyword_info) · tool_call は**自分で発行する**
