...
description: >-
  ...
  PathFinder 에이전트 분석틀로 키워드 간 이동 경로를 주요 경로(Top 5)와 핵심 분기점
  (Hub)으로 묶어, 검색 여정 흐름도·경로 카드·허브 카드로 흐름 유형·이탈/합류·검색량·
  인사이트를 제시합니다.
  "검색 여정 분석", "패스파인더 리포트", "고객 여정 분석", "{카테고리} 검색 경로",
  "검색 경로 리포트", "여정 리포트"를 요청할 때 사용하세요.
...
사용자가 검색 여정 분석을 요청하면 **반드시 `references/path-opportunity.md` 를 먼저 Read**
...
3. **시점**: `curr`(현재, 기본) / `3m`·`6m`·`9m`·`12m`.

시드가 이미 주어졌으면 바로 진행합니다. 국가·시점은 별도 언급이 없으면 `kr`·`curr` 입니다.
...
`{WORKDIR}/path-opportunity-report.html` — 자체완결 HTML (대시보드↔A4 토글 · 인쇄 PDF).
`{WORKDIR}` 는 실행 프로젝트의 `tmp/reports/listeningmind-path-opportunity-{시드}-{시각}/` 입니다.
...
[응답 도중 · 도구 호출마다]
  ② --type tool_call (path_finder · keyword_info 각각 발행)
...
### Step 4 · 도구 호출 · MCP 도구(path_finder · keyword_info) · tool_call 은 **직접 발행**
...
집계기(`_shared/render/*_aggregate.py`)는 MCP 응답을 그대로 읽는다.
...
MCP 도구(path_finder · keyword_info)는 optional `user_query` 파라미터를 받아 이력에 적재한다
