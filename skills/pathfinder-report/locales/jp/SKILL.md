---
name: lm-pathfinder-report-jp
description: >-
  ListeningMind MCP(path_finder · keyword_info)でシードキーワードの検索ジャーニー(Search
  Journey)を分析し、「検索ジャーニー分析」リッチ HTML レポート(ダッシュボード/A4)を生成します。
  PathFinder エージェントの分析フレームでキーワード間の移動経路を主要経路(Top 5)と中核の
  分岐点(Hub)にまとめ、検索ジャーニーのフロー図・経路カード・ハブカードで流れのタイプ・
  離脱/合流・検索ボリューム・インサイトを提示します。
  「検索ジャーニー分析」「パスファインダーレポート」「カスタマージャーニー分析」「{カテゴリ}の検索経路」
  「検索経路レポート」「ジャーニーレポート」を依頼されたときに使用してください。
...
# lm-pathfinder-report-jp — 検索ジャーニー分析レポート
...
ユーザーから検索ジャーニー分析を依頼されたら、**必ず `references/path-opportunity.md` を先に
...
1. **シードキーワード**: 分析対象(分析市場の言語で — jp なら日本語)
2. **国**: `jp`(既定)。kr/us はラベルを追加すれば対応。
3. **時点**: `curr`(現在、既定)/ `3m`・`6m`・`9m`・`12m`。

シードがすでに与えられていればそのまま進めます。国・時点は特に指定がなければ `jp`・`curr` です。
...
`{WORKDIR}/path-opportunity-report.html` — 自己完結型 HTML(ダッシュボード↔A4 切替 · 印刷 PDF)。
`{WORKDIR}` は実行プロジェクトの `tmp/reports/listeningmind-path-opportunity-{シード}-{時刻}/` です。
...
[応答の途中 · ツール呼び出しごと]
  ② --type tool_call (path_finder · keyword_info をそれぞれ発行)
...
SKILL_DIR=$(find ~/.claude/skills ~/.claude/plugins /mnt/skills /mnt/user-data \
    ...
    -maxdepth 4 -type d -name lm-pathfinder-report-jp 2>/dev/null \
    | grep -v '\.trash' | head -1)
...
### Step 4 · ツール呼び出し · MCP ツール(path_finder · keyword_info) · tool_call は**自分で発行する**
...
集計スクリプト(`_shared/render/query_aggregate.py`)は MCP の応答をそのまま読む。
したがって応答は**加工せず原文のまま**ファイルに存在していなければならない。

**原則 · ファイルはパスだけで受け渡す。** `mcp_cache.py store --file` も
`query_aggregate.py --raw` もパスを受け取って自分で読む。中身をあなたのコンテキストに
...
python3 "$SKILL_DIR/scripts/log_event.py" --type artifact_created --session-id "$SID" \
  --kind path-opportunity \
  --html-file "$WORKDIR/path-opportunity-report.html"
```
