---
name: lm-queryfinder-report-jp
description: >-
  ListeningMind MCP(intent_finder · keyword_info)でシードキーワードの関連クエリを分析し、
  「クエリ機会分析」リッチ HTML レポート(ダッシュボード/A4)を生成します。QueryFinder
  エージェントの分析フレームで関連クエリを検索目的とブランド/ノンブランドのグループに
  まとめ、検索意図・根拠キーワード・検索ボリューム・インサイトをカード形式で提示します。
  「クエリ機会分析」「クエリファインダーレポート」「関連クエリ分析」「{カテゴリ}のクエリ機会」
  「検索機会レポート」を依頼されたときに使用してください。
...
# lm-queryfinder-report-jp — クエリ機会分析レポート
...
ユーザーからクエリ機会分析を依頼されたら、**必ず `references/query-opportunity.md` を先に
...
`{WORKDIR}/query-opportunity-report.html` — 自己完結型 HTML(ダッシュボード↔A4 切替 · 印刷 PDF)。
`{WORKDIR}` は実行プロジェクトの `tmp/reports/listeningmind-query-opportunity-{シード}-{時刻}/` です。
...
[応答の途中 · ツール呼び出しごと]
  ② --type tool_call (intent_finder · keyword_info をそれぞれ発行)
...
SKILL_DIR=$(find ~/.claude/skills ~/.claude/plugins /mnt/skills /mnt/user-data \
    ...
    -maxdepth 4 -type d -name lm-queryfinder-report-jp 2>/dev/null \
    | grep -v '\.trash' | head -1)
...
### Step 4 · ツール呼び出し · MCP 4 ツール · tool_call は**自分で発行する**
